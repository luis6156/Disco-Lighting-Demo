...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, 2.5f, 1));
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(3, 1.5f, 0));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(10), glm::vec3(0, 0, 1));
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(100), glm::vec3(1, 1, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_2) && !isOrtho) {
        fov += 5 * deltaTime;
        projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, 0.01f, 200.0f);
    }

    if (window->KeyHold(GLFW_KEY_1) && !isOrtho) {
        fov -= 5 * deltaTime;
        projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, 0.01f, 200.0f);
    }
