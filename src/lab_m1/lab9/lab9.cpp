...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, angle_square, glm::vec3(0, 1, 0));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    if (texture2) {
        glUniform1i(loc_mix_textures, 1);
    }
    else {
        glUniform1i(loc_mix_textures, 0);
    }

    // BONUS 1
    int loc_rotate = glGetUniformLocation(shader->program, "rotate");
    if (texture1 == mapTextures["earth"]) {
        glUniform1i(loc_rotate, 1);

        int loc_time = glGetUniformLocation(shader->program, "time");
        glUniform1f(loc_time, Engine::GetElapsedTime());
    }
    else {
        glUniform1i(loc_rotate, 0);
    }
...
void Lab9::OnInputUpdate(float deltaTime, int mods)
{
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        // BONUS 2 - Pentru al doilea bonus am 2 implementari, pentru ca nu am inteles exact enuntul si nu-mi mai aminteam din simularea ta ce trebuia sa faca:((((:
        //  - in prima, cea comentata mai jos, patratul se roteste in functie de ritatia camerei (e mereu paralel cu vectorul right)
        //  - in cea de-a doua implementare, patratul 'priveste' mereu spre camera (e orientat perpendicular pe dreapta formata de pozitia camerei si patrat)

        /*
        glm::vec3 v1 = right;
        glm::vec3 v2 = glm::vec3(1, 0.5f, 0);
       
        angle_square = atan2(v2.z * v1.x - v2.x * v1.z, v1.x * v2.x + v1.z * v2.z);
        */

        glm::vec3 position = GetSceneCamera()->m_transform->GetLocalPosition();
        glm::vec3 v1 = position;
        glm::vec3 v2 = glm::vec3(1, 0.5f, 0);

        angle_square = atan2(v2.z * v1.x - v2.x * v1.z, v1.x * v2.x + v1.z * v2.z);
        angle_square += RADIANS(90);
    }
}
